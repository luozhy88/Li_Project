...
library(dplyr)
# 读取化合物信息
DF.TCM=readxl::read_xlsx("../../../../database/TCM/v20240929/input/TCMSP.xlsx",sheet = 1)
...
# DF.TCM_prescription=DF.CID.compound.gene %>% filter(herb_cn_name =="浙贝母|夏枯草|王不留行|蒲公英|海藻|玄参|白术|牡蛎|皂角刺")
DF.TCM_prescription=DF.CID.compound.gene %>% filter(grepl("浙贝母|夏枯草|丹皮|牡蛎|玄参|青皮|白术|牡蛎|皂角刺|当归|莪术|白芍|^柴胡|^茯苓|白术|黄药子",herb_cn_name) )
DF.TCM_prescription$herb_cn_name %>% unique()


write.csv(DF.TCM_zhebeimu,"output/01_TCM_CID_compound_Gene_ZBM.csv",row.names = FALSE,quote = T)
write.csv(DF.TCM_xiakucao,"output/01_TCM_CID_compound_Gene_XKC.csv",row.names = FALSE,quote = T)
write.csv(DF.TCM_prescription,"output/01_TCM_CID_compound_Gene_XYT.csv",row.names = FALSE,quote = T)
