...
analyze_tcm_data <- function(Filename) {
  ...
  Go_enrich_df$Type=Type
  Kegg_enrich_df=as.data.frame(kegg_enrich)
  Kegg_enrich_df$Type=Type
  write.csv(Go_enrich_df, glue("output/03_go_enrichment_results_{Type}.csv"), row.names = FALSE)
  write.csv(Kegg_enrich_df , glue("output/03_kegg_enrichment_results_{Type}.csv"), row.names = FALSE)
...
# XKC
Filename <- "output/02_common_gene_XKC.csv"
results <- analyze_tcm_data(Filename)

# ZBM
Filename <- "output/02_common_gene_ZBM.csv"
results <- analyze_tcm_data(Filename)

# all
Filename <- "output/02_common_gene_XYT.csv"
results <- analyze_tcm_data(Filename)
